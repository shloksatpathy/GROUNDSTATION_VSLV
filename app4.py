...
class Groundstation(QtWidgets.QMainWindow):
    ...
    def tick(self):
        ...
        # ----- Plot arrays -----
        times = [r["TIME_SINCE_S"] if "TIME_SINCE_S" in r else 0 for r in self.buffer]
        alt   = [r["ALTITUDE_M"] for r in self.buffer if "ALTITUDE_M" in r]
        pres  = [r["PRESSURE_PA"] for r in self.buffer if "PRESSURE_PA" in r]
        temp  = [r["TEMP_C"] for r in self.buffer if "TEMP_C" in r]

        # Update time-series plots with seconds (not packet index)
        if len(times) == len(alt):
            self.cur_alt.setData(times, alt)
        if len(times) == len(pres):
