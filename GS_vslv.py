...
# smoothing window for vertical speed (number of instantaneous samples)
...
class Groundstation(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.plot_alt = pg.PlotWidget(title="Altitude(m) vs Time")
        self.plot_pres = pg.PlotWidget(title="Pressure (Pa) vs Time")
        self.plot_temp = pg.PlotWidget(title="Temperature(c) vs Time")
        ...
        # ---------------- Vertical Speed plot (new) ----------------
        self.plot_vspeed = pg.PlotWidget(title="Vertical Speed vs Time (m/s)")
        self.plot_vspeed.setMinimumHeight(140)
        self.plot_vspeed.getPlotItem().showGrid(x=True, y=True, alpha=0.2)
        self.plot_vspeed.enableAutoRange(axis='x', enable=True)
        self.plot_vspeed.enableAutoRange(axis='y', enable=True)
        grid.addWidget(self.plot_vspeed, 6, 2)
        self.cur_vspeed = self.plot_vspeed.plot(pen=pg.mkPen(color=QColor(255, 100, 50), width=2))
        self.cur_vspeed.setClipToView(True)
        self.plot_vspeed.setLabel('bottom', 'Time (s)')
        self.plot_vspeed.setLabel('left', 'V (m/s)')

        # ---------------- Info Block (moved to col 3) ----------------
        ...
        grid.addWidget(info_widget, 6, 3)
        ...
        for curve in (self.cur_p_alt, self.cur_t_alt, self.cur_alt, self.cur_pres, self.cur_temp, self.cur_roll, self.cur_pitch, self.cur_yaw, self.cur_vspeed):
            curve.setClipToView(True)

        # label x-axis as seconds
        for pw in (self.plot_alt, self.plot_pres, self.plot_temp, self.plot_roll, self.plot_pitch, self.plot_yaw, self.plot_vspeed):
        ...
        for r in reversed(self.buffer):
            ...
            try:
                ...
                try:
                    ...
                    continue
            if val > 10000:  # heuristic: treat large numbers as ms
                val = val / 1000.0
            return val
        return None
                ...
                try:
                    ...
                    for code, label in FLIGHT_STATE_MAP.items():
                        if label.lower() == sval:
                            return label
                    return sval
        return FLIGHT_STATE_MAP.get(0, "idle")
    ...
    def _nice_range(self, values, min_pad=0.1, min_span=1.0):
        # kept for potential future manual ranging; not used in auto mode
        if not values:
            return 0.0, min_span
        vmin = min(values)
        vmax = max(values)
    ...
    def tick(self):
        ...
        dev_str = f"{latest_dev_time:.3f} s" if latest_dev_time is not None else "—"
        # IST now (UTC+5:30)
        ...
        for line in raw_lines:
            ...
            # --- pressure vs altitude (scatter; auto-range) ---
            try:
            ...
            # --- temp vs altitude (scatter; auto-range) ---
            try:
            ...
            try:
                # make sure these are always defined for downstream blocks
                alt_vals, pres_vals, temp_vals = [], [], []
            ...
            try:
                ...
                vs_y = []
                if times_w:
                    # alt_vals may be empty if upstream had no altitude; guard accordingly
                    if 'alt_vals' not in locals() or not alt_vals:
                        alt_vals = [None] * len(times_w)
                        for r in window_data:
                            if self.roles.get('alt') in r:
                                try:
                                    alt_vals.append(float(_num_re.search(str(r[self.roles.get('alt')])).group(0)))
                                except Exception:
                                    alt_vals.append(None)

                    paired_alt = [(times_w[i], alt_vals[i]) for i in range(len(alt_vals)) if alt_vals[i] is not None]
                    if len(paired_alt) >= 2:
                        inst_vs = []
                        inst_t = []
                        for i in range(1, len(paired_alt)):
                            t0p, a0p = paired_alt[i-1]
                            t1p, a1p = paired_alt[i]
                            dt_seg = t1p - t0p
                            if dt_seg <= 0:
                                continue
                            vs = (a1p - a0p) / dt_seg
                            ...
                            inst_t.append(t1p)
                        if inst_vs:
                            window = max(1, int(VSPEED_SMOOTH_WINDOW))
                            smoothed = []
                            for i in range(len(inst_vs)):
                                start = max(0, i - (window-1))
                                smoothed.append(sum(inst_vs[start:i+1]) / (i - start + 1))
                            vs_x = inst_t
                            vs_y = smoothed
                if vs_x and vs_y:
                    self.cur_vspeed.setData(vs_x, vs_y)
                else:
                    self.cur_vspeed.clear()
            except Exception as e:
                print("vspeed plot error:", e)
                try:
                    self.cur_vspeed.clear()
                except Exception:
                    pass
            ...
            try:
                ...
                def set_single_plot(curve, arr, plot_widget):
                    if any(v is not None for v in arr) and times_w:
                        ...
                        curve.setData(x, y)
                        # no manual ranges: auto-range is enabled in __init__
                    else:
                        curve.clear()

                set_single_plot(self.cur_roll, roll, self.plot_roll)
                set_single_plot(self.cur_pitch, pitch, self.plot_pitch)
                set_single_plot(self.cur_yaw,   yaw,  self.plot_yaw)
            except Exception as e:
