...
class Groundstation(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        for pw in (self.plot_p_alt, self.plot_t_alt, self.plot_combined, self.plot_rpy):
            pw.setMinimumHeight(220); pw.getPlotItem().showGrid(x=True, y=True, alpha=0.2)

        # place combined at top (spans all columns)
        grid.addWidget(self.plot_combined, 4, 0, 1, 4)

        # keep pressure/temp vs altitude on the left; map center-right
        grid.addWidget(self.plot_p_alt, 5, 0)
        grid.addWidget(self.plot_t_alt, 5, 1)

        # map in same row (right)
        self.map_view = QWebEngineView()
        grid.addWidget(self.map_view, 5, 2)
        self.map_view.setMinimumHeight(260)
        self.update_map(20.5900, 78.9600)

        # place RPY expanded to the lower part spanning all columns
        grid.addWidget(self.plot_rpy, 6, 0, 1, 4)
        ...
        # combined plot curves with colored pens
        self.plot_combined.addLegend()
        self.cur_comb_alt = self.plot_combined.plot(name='Altitude', pen=pg.mkPen(color=QColor(0,120,215), width=2))
        self.cur_comb_pres = self.plot_combined.plot(name='Pressure', pen=pg.mkPen(color=QColor(255,165,0), width=2, style=Qt.SolidLine))
        self.cur_comb_temp = self.plot_combined.plot(name='Temp', pen=pg.mkPen(color=QColor(0,200,0), width=2, style=Qt.DashLine))

        # rpy curves + legend with distinct colors
        self.plot_rpy.addLegend()
        self.cur_rpy_roll  = self.plot_rpy.plot(name='Roll', pen=pg.mkPen(color=QColor(220,20,60), width=2))  # crimson
        self.cur_rpy_pitch = self.plot_rpy.plot(name='Pitch', pen=pg.mkPen(color=QColor(199,21,133), width=2, style=Qt.DashLine))  # mediumvioletred
        self.cur_rpy_yaw   = self.plot_rpy.plot(name='Yaw', pen=pg.mkPen(color=QColor(0,206,209), width=2, style=Qt.DotLine))  # darkturquoise

        for curve in (self.cur_p_alt, self.cur_t_alt, self.cur_comb_alt, self.cur_comb_pres, self.cur_comb_temp, self.cur_rpy_roll, self.cur_rpy_pitch, self.cur_rpy_yaw):
            curve.setClipToView(True)

        # label x-axis as seconds
        self.plot_combined.setLabel('bottom', 'Time (s)')
        self.plot_rpy.setLabel('bottom', 'Time (s)')
    ...
    def tick(self):
        ...
        for line in raw_lines:
            ...
            # --- combined plot update (time on x) ---
            try:
                if times_w and (alt or pres or temp):
                    # update combined curves; we need alt/pres/temp aligned with times_w length;
                    # For combined, we pair by iterating window_data and extracting numeric fields per-record.
                    comb_alt = []
                    comb_pres = []
                    comb_temp = []
                    for r in window_data:
                        # map each record to numbers if possible, else append None (pyqtgraph ignores None)
                        ...
                        comb_alt.append(a); comb_pres.append(p); comb_temp.append(t)

                    # setData expects same-length lists for x and y; we filter per-series
                    # build x lists that match available y values
                    if any(a is not None for a in comb_alt):
                        x_a = [times_w[i] for i, a in enumerate(comb_alt) if a is not None]
                        y_a = [a for a in comb_alt if a is not None]
                        self.cur_comb_alt.setData(x_a, y_a)
                    else:
                        self.cur_comb_alt.clear()

                    if any(p is not None for p in comb_pres):
                        x_p = [times_w[i] for i, p in enumerate(comb_pres) if p is not None]
                        y_p = [p for p in comb_pres if p is not None]
                        self.cur_comb_pres.setData(x_p, y_p)
                    else:
                        self.cur_comb_pres.clear()

                    if any(t is not None for t in comb_temp):
                        x_t = [times_w[i] for i, t in enumerate(comb_temp) if t is not None]
                        y_t = [t for t in comb_temp if t is not None]
                        self.cur_comb_temp.setData(x_t, y_t)
                    else:
            ...
            try:
                ...
                # filter Nones and align x-values
                def set_rpy_curve(curve, arr):
                ...
                set_rpy_curve(self.cur_rpy_roll, roll)
                set_rpy_curve(self.cur_rpy_pitch, pitch)
                set_rpy_curve(self.cur_rpy_yaw, yaw)
                ...
                print("rpy plot error:", e)

            time_block_end(t_plot)

            # map update every 20 packets (kept unchanged) - robust lat/lon extraction
            try:
